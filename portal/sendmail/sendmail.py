...
#继承Thread，需要实现run方法
class SendHtmlEmail_cc(threading.Thread):
    """send html email"""
    def __init__(self, subject, html_content, send_from, to_list, fail_silently = False):
        self.subject = subject
        self.html_content = html_content
    ...
    def run(self):
        msg = EmailMessage(self.subject, self.html_content, self.send_from, bcc=self.to_list)
        msg.content_subtype = "html" # Main content is now text/html
        msg.send(self.fail_silently)

def send_email_by_template_cc(subject, module, data, to_list):
    """
        使用模版发送邮件，发件人默认使用配置的
        subject: string, 主题
        module:  string, 模版名称
    ...
    send_email = SendHtmlEmail_cc(subject, html_content, send_from, to_list)
    send_email.start() #开启线程，自动运行线程里面的run方法
